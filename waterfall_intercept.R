waterfall_intercept <- function (.data = NULL, values, labels, rect_text_labels = values, 
          rect_text_size = 1, rect_text_fontface = "plain",rect_text_labels_anchor = "centre", 
          ...
          print_plot = FALSE, ggplot_object_name = "mywaterfall",intercept=0,intercept_colour="black",alpha=1,
          text_inside_rect_col="black",text_outside_rect_col="black") 
{
  if (!is.null(.data)) {
  ...
  south_edge <- c(intercept, cumsum(values)[-length(values)])
  ...
  if (!calc_total) {
    ...
    p <- if (scale_y_to_waterfall) {
      ggplot2::ggplot(data.frame(x = c(labels, total_axis_text, 
                                       labels, total_axis_text), y = c(south_edge, 
                                                                       north_edge, south_edge[number_of_rectangles], 
                                                                       north_edge[number_of_rectangles])), ggplot2::aes_string(x = "x", 
                                                                                                                               y = "y"))
    }
    else {
      ggplot2::ggplot(data.frame(x = c(labels, total_axis_text), 
  ...
  if (grepl("behind", draw_axis.x)) {
    p <- p + ggplot2::geom_hline(yintercept = intercept,col = intercept_colour) # MOVE INTERCEPT LINE
  }
  for (i in seq_along(values)) {
    p <- p + ggplot2::annotate("rect", xmin = i - rect_width/2, 
                               xmax = i + rect_width/2, ymin = south_edge[i], ymax = north_edge[i], 
                               colour = rect_border, fill = fill_colours[i], alpha=alpha) ## ADD ALPHA TO PLOT
    if (i > 1 && draw_lines) {
  ...
  for (i in seq_along(values)) {
    if (abs(values[i]) > put_rect_text_outside_when_value_below) {
      p <- p + ggplot2::annotate("text", x = i, y = 0.5 * 
                                   (north_edge[i] + south_edge[i]), family = theme_text_family, fontface = rect_text_fontface,
                                 ...
                                 col=text_inside_rect_col)
    }
    else {
      p <- p + ggplot2::annotate("text", x = i, y = north_edge[i], 
                                 family = theme_text_family,  fontface = rect_text_fontface,
                                 label = ifelse(rect_text_labels[i] == 
                                 ...
                                 col=text_outside_rect_col)
    }
  }
  if (calc_total) {
    p <- p + ggplot2::annotate("rect", xmin = number_of_rectangles + 
                                 1 - rect_width/2, xmax = number_of_rectangles + 
                                 1 + rect_width/2, ymin = intercept, ymax = north_edge[number_of_rectangles], 
  ...
  if (grepl("front", draw_axis.x)) {
    p <- p + ggplot2::geom_hline(yintercept = intercept,col = intercept_colour)
  }
  if (print_plot) {
